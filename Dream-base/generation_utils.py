...
class DreamGenerationMixin:
    ...
    def _sample(
        ...
        for i in range(steps):
            ...
            if alg == 'origin':
                ...
                # 计算熵
                probs_for_entropy = torch.softmax(mask_logits, dim=-1)
                entropy = -torch.sum(probs_for_entropy * (probs_for_entropy + 1e-8).log(), dim=-1)
                entropy = entropy / np.log(probs_for_entropy.shape[-1])

                # 使用融合模块计算语义置信度
                if hasattr(self, '_fuse_confidence'):
                    confidence = self._fuse_confidence(confidence, entropy, pmass_for_masks)
    ...
    def _slow_fast_sample(
        ...
        while True:
            ...
            # ============================================================
            # 初始化 start_cycle_idx 和 start_abs_idx，防止未赋值报错
            start_cycle_idx = 0
            start_abs_idx = prompt_length
            for discovery_step in range(N_CYCLE_DISCOVERY_STEPS):
                ...
                for b_idx in range(batch_size):
                    ...
                    # === 后续所有置信度判断都用 semantic_conf ===
                    confidence_undecoded_cycle = torch.where(mask_positions_bool_p0[b_idx, start_abs_idx:], semantic_conf[start_cycle_idx:], torch.tensor(-np.inf, device=x.device, dtype=semantic_conf.dtype))
            ...
            while True: # Loop for Phase 1 steps within this major_cycle
                ...
                for b_idx in range(batch_size):
                    ...
                    if hasattr(self, '_fuse_confidence'):
                        ...
                        semantic_conf_p1 = confidence_p1
                    # === 后续所有置信度判断都用 semantic_conf_p1 ===
                    confidence_undecoded_in_cycle = torch.where(mask_positions_bool_p1[b_idx, start_abs_idx:end_abs_idx], semantic_conf_p1[start_cycle_idx:end_cycle_idx], torch.tensor(-np.inf, device=x.device, dtype=semantic_conf_p1.dtype))
