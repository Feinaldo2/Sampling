        ...
        for block_idx in range(num_blocks):
            ...
            while True:
                ...
                for k_step in range(k_exploration_steps):
                    ...
                    conf, x0_gen = p_gen.max(dim=-1)
                    # 计算熵和 pmass
                    entropy = -torch.sum(p_gen * (p_gen + 1e-8).log(), dim=-1)
                    entropy = normalize_entropy(entropy, p_gen.shape[-1])
                    pmass = torch.zeros_like(conf)  # 这里需要 attention weights 支持，否则置零
                    # 融合置信度
                    if hasattr(model, '_fuse_confidence'):
                        semantic_conf = model._fuse_confidence(conf, entropy, pmass)
                    else:
                        semantic_conf = conf
                    # 调试输出
                    if int(os.environ.get("RANK", "0")) == 0:
                        print("use_attention_fusion:", getattr(model, 'use_attention_fusion', None))
                        print("static_weight:", getattr(model, 'static_weight', None))
                        print("conf mean:", conf.mean().item(), "entropy mean:", entropy.mean().item(), "pmass mean:", pmass.mean().item())
                        print("semantic_conf mean:", semantic_conf.mean().item())
                        with open("debug_print.txt", "a") as f:
                            f.write(f"use_attention_fusion: {getattr(model, 'use_attention_fusion', None)}\n")
                            f.write(f"static_weight: {getattr(model, 'static_weight', None)}\n")
                            f.write(f"conf mean: {conf.mean().item()}, entropy mean: {entropy.mean().item()}, pmass mean: {pmass.mean().item()}\n")
                            f.write(f"semantic_conf mean: {semantic_conf.mean().item()}\n\n")
                    current_global_mask_index_gen_part = (x[:, prompt_length:] == mask_id) 
                    confidence_gen_wide = torch.where(current_global_mask_index_gen_part, semantic_conf, torch.tensor(-np.inf, device=x.device, dtype=semantic_conf.dtype))
