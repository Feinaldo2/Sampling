...
class Dream(LM):
    ...
    def _create_model_and_tokenizer(self, pretrained, dtype, trust_remote_code):
        ...
        # ✅ 配置注意力融合参数（在模型创建后）
        if hasattr(self.model, 'semantic_weight_attention'):
            eval_logger.info("✅ Semantic weight attention module loaded successfully")
            
            # 设置融合类型和权重
            self.model.use_attention_fusion = self.use_attention_fusion
            if self.fusion_type == "static":
                self.model.static_fusion_weights = self.static_weight
                self.model.use_static_fusion = True
                # 也要设置到语义注意力模块中
            ...
            eval_logger.info(f"Attention fusion enabled: {self.use_attention_fusion}")
            eval_logger.info(f"Fusion type: {self.fusion_type}")
            if self.fusion_type == "static":
                eval_logger.info(f"Static weights: {self.static_weight}")
        else:
            eval_logger.warning("❌ Semantic weight attention module NOT found in model")
    ...
    def _generate_batch(self, prompts: List[str]) -> List[str]:
        ...
        if isinstance(result, tuple) and len(result) == 3:
            generation_ids, avg_model_calls_length, global_model_calls = result
        else:
