...
class DummySemanticWeightModule(torch.nn.Module):
    def forward(self, features):
        N = features.shape[0]
        return torch.tensor([[0.6, 0.2, 0.2]] * N, dtype=features.dtype, device=features.device)

# Dummy tensor parallel for multi-GPU: 单卡/数据并行时直接返回原模型
...
class LLaDA(TemplateLM):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if use_static_weight:
            self.semantic_weight_module = DummySemanticWeightModule()
        else:
            self.semantic_weight_module = SemanticWeightSelfAttention(d_model=32, nhead=4, num_layers=1)
            if semantic_weight_path is not None:
                self.semantic_weight_module.load(semantic_weight_path, map_location=self.device)
    ...
    def generate_until(self, requests: List[Instance]) -> List[str]:
        ...
        for batch in ds.iter(self.batch_size):
            ...
            sampler=SlowFastSampler(model=self.model,gen_kwargs=gen_kwargs, semantic_weight_module=self.semantic_weight_module)
